...
import edu.bluejack20_2.Konnect.view.fragment.ChatListFragment
...
class ChatDetailActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        Log.wtf("data", "testasdfasdfasdf")
        if (chatDetail != null) {
            Log.wtf("data", chatDetail.connectionDoc as String)
        }
        if (chatDetail != null) {
            Log.wtf("data saat masuk ke chat detail", "kesini ga lu")
            ChatRepository.loadChatText(chatDetail.chatRoomDoc as String).addSnapshotListener{ documentSnapshot, e ->
                if (e != null) {
...
//                Log.wtf("ada ga datanya? " , documentSnapshot.isEmpty.toString())
        ...
        btn.setOnClickListener(View.OnClickListener {
            val userRef = FirebaseFirestore.getInstance().collection("users").document(FirebaseAuth.getInstance().currentUser.uid)
            val toRef = FirebaseFirestore.getInstance().collection("users").document(chatDetail?.connectionDoc as String)
            val headerRef = FirebaseFirestore.getInstance().collection("chat_rooms").document(chatDetail?.chatRoomDoc.toString())
